...
def get_player_projections(
    ...
    def _fallback(reason: str) -> list[dict]:
        # One line per fallback, so a silent degrade to weighted averages is
        # always visible in the logs without narrating every healthy request.
    ...
    if not model_exists():
        return _fallback("no trained model artifact found")
